...
// Event Table
// ============================================================================
wxBEGIN_EVENT_TABLE(AnxietyMonitorPlugin,
                    cbPlugin) EVT_TIMER(ID_ANXIETY_TIMER,
                                        AnxietyMonitorPlugin::OnTimerUpdate)
    EVT_BUTTON(ID_ANXIETY_START, AnxietyMonitorPlugin::OnStartButton)
        EVT_BUTTON(ID_ANXIETY_PAUSE, AnxietyMonitorPlugin::OnPauseButton)
            EVT_BUTTON(ID_ANXIETY_EXPORT, AnxietyMonitorPlugin::OnExportButton)
                EVT_BUTTON(ID_ANXIETY_SETTINGS,
                           AnxietyMonitorPlugin::OnSettingsButton)
                    EVT_MENU(ID_MENU_START, AnxietyMonitorPlugin::OnMenuStart)
                        EVT_MENU(ID_MENU_PAUSE,
                                 AnxietyMonitorPlugin::OnMenuPause)
                            EVT_MENU(ID_MENU_END,
                                     AnxietyMonitorPlugin::OnMenuEnd)
                                EVT_MENU(ID_MENU_SHOW_PANEL,
                                         AnxietyMonitorPlugin::OnMenuShowPanel)
                                    wxEND_EVENT_TABLE()
...
#else
    // Export required functions for CodeBlocks plugin loading
    // These are required even without full SDK to allow CodeBlocks to load the
    // DLL
    extern "C" {
  // Global plugin instance
  static AnxietyMonitorPlugin *g_pluginInstance = nullptr;

// GetPlugin - CodeBlocks calls this to get the plugin instance
#ifdef _WIN32
  __declspec(dllexport) cbPlugin *GetPlugin()
#else
  cbPlugin *GetPlugin()
#endif
  {
    if (!g_pluginInstance) {
      g_pluginInstance = new AnxietyMonitorPlugin();
    }
    return g_pluginInstance;
  }

// GetSDKVersion - CodeBlocks calls this to verify SDK compatibility
// Return format: major * 10000 + minor * 100 + release
// CodeBlocks 25.03 uses SDK 2.0.0
#ifdef _WIN32
...
#endif
  {
    // Return SDK version 2.0.0 (20000) for CodeBlocks 25.03
    return 20000; // major=2, minor=0, release=0
  }
}
#endif
...
AnxietyMonitorPlugin::AnxietyMonitorPlugin()
    : m_panel(nullptr), m_toolBar(nullptr), m_startButtonId(ID_ANXIETY_START),
      m_pauseButtonId(ID_ANXIETY_PAUSE), m_exportButtonId(ID_ANXIETY_EXPORT),
      m_settingsButtonId(ID_ANXIETY_SETTINGS),
      m_updateTimer(this, ID_ANXIETY_TIMER),
      m_sessionState(SessionState::STOPPED), m_isInitialized(false) {
...
void AnxietyMonitorPlugin::OnRelease(bool appShutDown) {
  wxLogMessage("AnxietyMonitor: Plugin releasing... (shutdown=%d)",
               appShutDown);
...
bool AnxietyMonitorPlugin::BuildToolBar(wxToolBar *toolBar) {
  ...
  // Connect events
  toolBar->Bind(wxEVT_TOOL, &AnxietyMonitorPlugin::OnStartButton, this,
                ID_ANXIETY_START);
  toolBar->Bind(wxEVT_TOOL, &AnxietyMonitorPlugin::OnPauseButton, this,
                ID_ANXIETY_PAUSE);
  toolBar->Bind(wxEVT_TOOL, &AnxietyMonitorPlugin::OnExportButton, this,
                ID_ANXIETY_EXPORT);
  toolBar->Bind(wxEVT_TOOL, &AnxietyMonitorPlugin::OnSettingsButton, this,
                ID_ANXIETY_SETTINGS);
...
void AnxietyMonitorPlugin::BuildModuleMenu(const ModuleType type, wxMenu *menu,
...
void AnxietyMonitorPlugin::StartSession() {
  ...
  wxLogMessage("AnxietyMonitor: Session started - %s",
...
void AnxietyMonitorPlugin::ExportSession() {
  ...
  // Show file dialog
  wxFileDialog saveDialog(nullptr, "Export Session Data", wxEmptyString,
                          wxEmptyString, "CSV files (*.csv)|*.csv",
...
void AnxietyMonitorPlugin::ShowSettings() {
  ...
  wxMessageBox("Anxiety Monitor Settings\n\n"
               ...
               "Anxiety Monitor Settings", wxOK | wxICON_INFORMATION);
}
...
void AnxietyMonitorPlugin::ForceSave() {
  ...
  if (m_sessionState != SessionState::STOPPED && m_csvWriter &&
      m_dataCollector) {
    MetricsSnapshot snapshot = m_dataCollector->GetCurrentSnapshot();
    if (m_scorer) {
      snapshot.anxietyScore = m_scorer->CalculateScore(snapshot);
      snapshot.riskLevel =
          GetRiskLevelLabel(m_scorer->GetRiskLevel(snapshot.anxietyScore));
    }
    m_csvWriter->WriteSnapshot(snapshot);
    m_csvWriter->Flush();
...
void AnxietyMonitorPlugin::UpdateStatusBar() {
  ...
  if (m_sessionState == SessionState::STOPPED) {
    // Set idle status
    // SetStatusText("[Anxiety Monitor] Ready - Click Start to begin
    // monitoring");
    return;
  }
...
// ============================================================================
// CB SDK Event Handlers
// ============================================================================
#ifdef CODEBLOCKS_SDK_INCLUDED
void AnxietyMonitorPlugin::OnEditorEvent(CodeBlocksEvent &event) {
  if (m_eventHandlers)
    m_eventHandlers->OnEditorUpdateUI(event);
  else
    event.Skip();
}

void AnxietyMonitorPlugin::OnCompilerStart(CodeBlocksEvent &event) {
  if (m_eventHandlers)
    m_eventHandlers->OnCompilerStart(event);
  else
    event.Skip();
}

void AnxietyMonitorPlugin::OnCompilerFinished(CodeBlocksEvent &event) {
  if (m_eventHandlers)
    m_eventHandlers->OnCompilerFinished(event);
  else
    event.Skip();
}

void AnxietyMonitorPlugin::OnAppStartupDone(CodeBlocksEvent &event) {
  if (m_eventHandlers)
    m_eventHandlers->OnAppStartupDone(event);
  else
    event.Skip();
}
#endif
